...
from tratandoErros import trata_entrada_de_valor, tratamento_de_retorno
from conexaoBanco import cursor, conexao
from datetime import timedelta
import time

def tratando_custo_existente(data_inicial, data_final, data_existente):

    print(f"Já foi registrado um custo para próxima semana ...  no valor total de R${data_existente[3] * 6}", "\n")
    stop = False
    while not stop:
        print("Nesse caso o que deseja fazer ? \n")
        print("1 - Excluir o custo dessa semana e adicionar outro. \n"
              "2 - Não é necessário atualização, apenas volte ao menu principal. \n")

        opcao = input("Digite uma das opções acima: \n")
        if opcao == "1":
            novo_custo = input("Digite o novo valor de custo da próxima semana:\n")
            ...
            stop = tratamento_de_retorno(stop)


def adiciona_custo_semanal():

    data_inicial, data_final = fr.calcula_proximo_periodo_semanal()
    data_existente = ce.consulta_existencia_de_data(data_inicial)  # verifica existência de data

    if data_existente is not None:

        tratando_custo_existente(data_inicial, data_final, data_existente)

    novo_custo = input("Digite valor de custo da próxima semana:\n")
    novo_custo = trata_entrada_de_valor(novo_custo)
